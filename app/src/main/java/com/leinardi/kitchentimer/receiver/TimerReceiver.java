/*
 * Copyright (C) 2012 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.leinardi.kitchentimer.receiver;

import android.app.AlarmManager;
import android.app.Notification;
import android.app.PendingIntent;
import android.content.BroadcastReceiver;
import android.content.Context;
import android.content.Intent;
import android.content.SharedPreferences;
import android.os.Build;
import android.preference.PreferenceManager;
import android.support.v4.app.NotificationCompat;
import android.support.v4.app.NotificationManagerCompat;
import android.text.format.DateUtils;
import android.util.Log;

import com.leinardi.kitchentimer.R;
import com.leinardi.kitchentimer.model.Countdown;
import com.leinardi.kitchentimer.service.TimerRingService;
import com.leinardi.kitchentimer.ui.MainActivity;
import com.leinardi.kitchentimer.utils.LogUtils;
import com.leinardi.kitchentimer.utils.Utils;

import java.util.Iterator;

import io.realm.Realm;
import io.realm.RealmResults;

public class TimerReceiver extends BroadcastReceiver {
    private static final String TAG = TimerReceiver.class.getSimpleName();

    // Private actions processed by the receiver
    public static final String START_TIMER = "start_timer";
    public static final String DELETE_TIMER = "delete_timer";
    public static final String TIMES_UP = "times_up";
    public static final String TIMER_RESET = "timer_reset";
    public static final String TIMER_STOP = "timer_stop";
    //    public static final String TIMER_DONE = "timer_done";
    public static final String TIMER_UPDATE = "timer_update";

    public static final String TIMER_INTENT_EXTRA = "timer.intent.extra";

    public static final String NOTIF_IN_USE_SHOW = "notif_in_use_show";
    public static final String NOTIF_IN_USE_CANCEL = "notif_in_use_cancel";
    public static final String NOTIF_APP_OPEN = "notif_app_open";
    public static final String FROM_NOTIFICATION = "from_notification";
    public static final String NOTIF_TIMES_UP_STOP = "notif_times_up_stop";
    public static final String NOTIF_TIMES_UP_PLUS_ONE = "notif_times_up_plus_one";
    public static final String NOTIF_TIMES_UP_SHOW = "notif_times_up_show";
    public static final String NOTIF_TIMES_UP_CANCEL = "notif_times_up_cancel";

    // Make this a large number to avoid the alarm ID's which seem to be 1, 2, ...
    // Must also be different than StopwatchService.NOTIFICATION_ID
    private static final int IN_USE_NOTIFICATION_ID = Integer.MAX_VALUE - 2;

    private Realm mRealm;

    @Override
    public void onReceive(final Context context, final Intent intent) {
        LogUtils.d(TAG, "Received intent " + intent.toString());

        String actionType = intent.getAction();
        // This action does not need the timers data
        if (NOTIF_IN_USE_CANCEL.equals(actionType)) {
            cancelInUseNotification(context);
            return;
        }

        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);

        // Get the updated timers data.
        mRealm = Countdown.getRealmInstance(context);

        // These actions do not provide a timer ID, but do use the timers data
        if (NOTIF_IN_USE_SHOW.equals(actionType)) {
            showInUseNotification(context);
            return;
        } else if (NOTIF_TIMES_UP_SHOW.equals(actionType)) {
            showTimesUpNotification(context);
            return;
        } else if (NOTIF_TIMES_UP_CANCEL.equals(actionType)) {
            cancelTimesUpNotification(context);
            return;
        } else if (Intent.ACTION_BOOT_COMPLETED.equals(actionType)) {
            Log.d(TAG, "ACTION_BOOT_COMPLETED - restoring timers");
            showInUseNotification(context);
            updateNextTimesup(context);
            return;
        }

        // Remaining actions provide a timer Id
        if (!intent.hasExtra(TIMER_INTENT_EXTRA)) {
            // No data to work with, do nothing
            LogUtils.e(TAG, "got intent without Timer data");
            return;
        }

        // Get the timer out of the Intent
        int timerId = intent.getIntExtra(TIMER_INTENT_EXTRA, -1);
        if (timerId == -1) {
            LogUtils.d(TAG, "OnReceive:intent without Timer data for " + actionType);
        }

        Countdown t = findTimer(timerId);

        if (TIMES_UP.equals(actionType)) {
            // Find the timer (if it doesn't exists, it was probably deleted).
            if (t == null) {
                LogUtils.d(TAG, " timer not found in list - do nothing");
                return;
            }

            mRealm.beginTransaction();
            t.setState(Countdown.STATE_TIMESUP);
            mRealm.commitTransaction();
            // Play ringtone by using TimerRingService service with a default alarm.
            LogUtils.d(TAG, "playing ringtone");
            Intent si = new Intent();
            si.setClass(context, TimerRingService.class);
            context.startService(si);

            // Update the in-use notification
            if (getNextRunningTimer(false, Utils.getTimeNow()) == null) {
                // Found no running timers.
                cancelInUseNotification(context);
            } else {
                showInUseNotification(context);
            }

            // Start the MainActivity activity.
            Intent timersAlert = new Intent(context, MainActivity.class);
            timersAlert.putExtra(MainActivity.EXTRA_TIMES_UP, true);
            timersAlert.setFlags(
                    Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NO_USER_ACTION);
            context.startActivity(timersAlert);
        } else if (TIMER_RESET.equals(actionType)
//                || TIMER_DONE.equals(actionType)
                || DELETE_TIMER.equals(actionType)) {
            // Stop Ringtone if all timers are not in times-up status
            stopRingtoneIfNoTimesup(context);
        } else if (NOTIF_TIMES_UP_STOP.equals(actionType)) {
            // Find the timer (if it doesn't exists, it was probably deleted).
            if (t == null) {
                LogUtils.d(TAG, "timer to stop not found in list - do nothing");
                return;
            } else if (t.getState() != Countdown.STATE_TIMESUP) {
                LogUtils.d(TAG, "action to stop but timer not in times-up state - do nothing");
                return;
            }

            cancelTimesUpNotification(context, t);

            // Done with timer - delete from data base
            mRealm.beginTransaction();
            t.removeFromRealm();
            mRealm.commitTransaction();


            // Stop Ringtone if no timers are in times-up status
            stopRingtoneIfNoTimesup(context);
        } else if (NOTIF_TIMES_UP_PLUS_ONE.equals(actionType)) {
            // Find the timer (if it doesn't exists, it was probably deleted).
            if (t == null) {
                LogUtils.d(TAG, "timer to +1m not found in list - do nothing");
                return;
            } else if (t.getState() != Countdown.STATE_TIMESUP) {
                LogUtils.d(TAG, "action to +1m but timer not in times up state - do nothing");
                return;
            }

            // Restarting the timer with 1 minute left.
            mRealm.beginTransaction();
            t.setState(Countdown.STATE_RUNNING);
            t.setStartTime(Utils.getTimeNow());
            t.setTimeLeft(DateUtils.MINUTE_IN_MILLIS);
            t.setOriginalLength(DateUtils.MINUTE_IN_MILLIS);
            mRealm.commitTransaction();

            cancelTimesUpNotification(context, t);

            // If the app is not open, refresh the in-use notification
            if (!prefs.getBoolean(NOTIF_APP_OPEN, false)) {
                showInUseNotification(context);
            }

            // Stop Ringtone if no timers are in times-up status
            stopRingtoneIfNoTimesup(context);
        } else if (TIMER_UPDATE.equals(actionType)) {
            // Find the timer (if it doesn't exists, it was probably deleted).
            if (t == null) {
                LogUtils.d(TAG, " timer to update not found in list - do nothing");
                return;
            }

            // Refresh buzzing notification
            if (t.getState() == Countdown.STATE_TIMESUP) {
                // Must cancel the previous notification to get all updates displayed correctly
                cancelTimesUpNotification(context, t);
                showTimesUpNotification(context, t);
            }
        }
        // Update the next "Times up" alarm
        updateNextTimesup(context);

        mRealm.close();
    }

    private Countdown findTimer(int timerId) {
        return mRealm.where(Countdown.class)
                .equalTo("timerId", timerId)
                .findFirst();
    }

    private void stopRingtoneIfNoTimesup(final Context context) {
        if (findExpiredTimer() == null) {
            // Stop ringtone
            LogUtils.d(TAG, "stopping ringtone");
            Intent si = new Intent();
            si.setClass(context, TimerRingService.class);
            context.stopService(si);
        }
    }

    private Countdown findExpiredTimer() {
        return mRealm.where(Countdown.class)
                .equalTo("state", Countdown.STATE_TIMESUP)
                .findFirst();
    }

    // Scan all timers and find the one that will expire next.
    // Tell AlarmManager to send a "Time's up" message to this receiver when this timer expires.
    // If no timer exists, clear "time's up" message.
    private void updateNextTimesup(Context context) {
        Countdown t = getNextRunningTimer(false, Utils.getTimeNow());
        long nextTimesup = (t == null) ? -1 : getTimesupTime(t);
        int timerId = (t == null) ? -1 : t.getTimerId();

        Intent intent = new Intent();
        intent.setAction(TIMES_UP);
        intent.setClass(context, TimerReceiver.class);
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            // Time-critical, should be foreground
            intent.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        }
        if (!mRealm.where(Countdown.class).findAll().isEmpty()) {
            intent.putExtra(TIMER_INTENT_EXTRA, timerId);
        }
        AlarmManager mngr = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        PendingIntent p = PendingIntent.getBroadcast(context,
                0, intent, PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_UPDATE_CURRENT);
        if (t != null) {
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2) {
                mngr.setExact(AlarmManager.RTC_WAKEUP, nextTimesup, p);
            } else {
                mngr.set(AlarmManager.RTC_WAKEUP, nextTimesup, p);
            }
            LogUtils.d(TAG, "Setting times up to " + nextTimesup);
        } else {
            mngr.cancel(p);
            LogUtils.d(TAG, "no next times up");
        }
    }

    private void showInUseNotification(final Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        boolean appOpen = prefs.getBoolean(NOTIF_APP_OPEN, false);
        RealmResults<Countdown> timersInUse = timersInUse();
        int numTimersInUse = timersInUse.size();

        if (appOpen || numTimersInUse == 0) {
            return;
        }

        String title, contentText;
        Long nextBroadcastTime = null;
        long now = Utils.getTimeNow();
        if (timersInUse.size() == 1) {
            Countdown timer = timersInUse.get(0);
            boolean timerIsTicking = Countdown.isTicking(timer);
            String label = timer.getLabel();
            title = timerIsTicking ? label : context.getString(R.string.timer_stopped);
            long timeLeft = timerIsTicking ? getTimesupTime(timer) - now : timer.getTimeLeft();
            contentText = buildTimeRemaining(context, timeLeft);
            if (timerIsTicking && timeLeft > DateUtils.MINUTE_IN_MILLIS) {
                nextBroadcastTime = getBroadcastTime(now, timeLeft);
            }
        } else {
            Countdown timer = getNextRunningTimer(false, now);
            if (timer == null) {
                // No running timers.
                title = String.format(
                        context.getString(R.string.timers_stopped), numTimersInUse);
                contentText = context.getString(R.string.all_timers_stopped_notif);
            } else {
                // We have at least one timer running and other timers stopped.
                title = String.format(
                        context.getString(R.string.timers_in_use), numTimersInUse);
                long completionTime = getTimesupTime(timer);
                long timeLeft = completionTime - now;
                contentText = String.format(context.getString(R.string.next_timer_notif),
                        buildTimeRemaining(context, timeLeft));
                if (timeLeft <= DateUtils.MINUTE_IN_MILLIS) {
                    Countdown timerWithUpdate = getNextRunningTimer(true, now);
                    if (timerWithUpdate != null) {
                        completionTime = getTimesupTime(timerWithUpdate);
                        timeLeft = completionTime - now;
                        nextBroadcastTime = getBroadcastTime(now, timeLeft);
                    }
                } else {
                    nextBroadcastTime = getBroadcastTime(now, timeLeft);
                }
            }
        }
        showCollapsedNotificationWithNext(context, title, contentText, nextBroadcastTime);
    }

    private RealmResults<Countdown> timersInUse() {
        return mRealm.where(Countdown.class)
                .equalTo("state", Countdown.STATE_RUNNING)
                .or()
                .equalTo("state", Countdown.STATE_STOPPED)
                .findAll();
    }

    private long getBroadcastTime(long now, long timeUntilBroadcast) {
        long seconds = timeUntilBroadcast / 1000;
        seconds = seconds - ((seconds / 60) * 60);
        return now + (seconds * 1000);
    }

    private void showCollapsedNotificationWithNext(
            final Context context, String title, String text, Long nextBroadcastTime) {
        Intent activityIntent = new Intent(context, MainActivity.class);
        activityIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        PendingIntent pendingActivityIntent = PendingIntent.getActivity(context, 0, activityIntent,
                PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_UPDATE_CURRENT);
        showCollapsedNotification(context, title, text, NotificationCompat.PRIORITY_HIGH,
                pendingActivityIntent, IN_USE_NOTIFICATION_ID, false);

        if (nextBroadcastTime == null) {
            return;
        }
        Intent nextBroadcast = new Intent();
        nextBroadcast.setAction(NOTIF_IN_USE_SHOW);
        PendingIntent pendingNextBroadcast =
                PendingIntent.getBroadcast(context, 0, nextBroadcast, 0);
        AlarmManager alarmManager =
                (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2) {
            alarmManager.setExact(AlarmManager.RTC, nextBroadcastTime, pendingNextBroadcast);
        } else {
            alarmManager.set(AlarmManager.RTC, nextBroadcastTime, pendingNextBroadcast);
        }
    }

    private static void showCollapsedNotification(final Context context, String title, String text,
                                                  int priority, PendingIntent pendingIntent, int notificationId, boolean showTicker) {
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context)
                .setAutoCancel(false)
                .setContentTitle(title)
                .setContentText(text)
                .setDeleteIntent(pendingIntent)
                .setOngoing(true)
                .setPriority(priority)
                .setShowWhen(false)
                .setSmallIcon(R.drawable.stat_notify_timer)
                .setCategory(NotificationCompat.CATEGORY_ALARM)
                .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
                .setLocalOnly(true);
        if (showTicker) {
            builder.setTicker(text);
        }

        Notification notification = builder.build();
        notification.contentIntent = pendingIntent;
        NotificationManagerCompat notificationManager = NotificationManagerCompat.from(context);
        notificationManager.notify(notificationId, notification);
    }

    private String buildTimeRemaining(Context context, long timeLeft) {
        if (timeLeft < 0) {
            // We should never be here...
            LogUtils.v(TAG, "Will not show notification for timer already expired.");
            return null;
        }

        long hundreds, seconds, minutes, hours;
        seconds = timeLeft / 1000;
        minutes = seconds / 60;
        seconds = seconds - minutes * 60;
        hours = minutes / 60;
        minutes = minutes - hours * 60;
        if (hours > 99) {
            hours = 0;
        }

        String hourSeq = (hours == 0) ? "" :
                ((hours == 1) ? context.getString(R.string.hour) :
                        context.getString(R.string.hours, Long.toString(hours)));
        String minSeq = (minutes == 0) ? "" :
                ((minutes == 1) ? context.getString(R.string.minute) :
                        context.getString(R.string.minutes, Long.toString(minutes)));

        boolean dispHour = hours > 0;
        boolean dispMinute = minutes > 0;
        int index = (dispHour ? 1 : 0) | (dispMinute ? 2 : 0);
        String[] formats = context.getResources().getStringArray(R.array.timer_notifications);
        return String.format(formats[index], hourSeq, minSeq);
    }

    private Countdown getNextRunningTimer(
            boolean requireNextUpdate, long now) {
        long nextTimesup = Long.MAX_VALUE;
        boolean nextTimerFound = false;
        RealmResults<Countdown> countdownsRunning = mRealm.where(Countdown.class)
                .equalTo("state", Countdown.STATE_RUNNING)
                .findAll();
        Iterator<Countdown> i = countdownsRunning.iterator();
        Countdown t = null;
        while (i.hasNext()) {
            Countdown tmp = i.next();
            long timesupTime = getTimesupTime(tmp);
            long timeLeft = timesupTime - now;
            if (timesupTime < nextTimesup && (!requireNextUpdate || timeLeft > 60)) {
                nextTimesup = timesupTime;
                nextTimerFound = true;
                t = tmp;
            }
        }
        if (nextTimerFound) {
            return t;
        } else {
            return null;
        }
    }

    private void cancelInUseNotification(final Context context) {
        NotificationManagerCompat notificationManager = NotificationManagerCompat.from(context);
        notificationManager.cancel(IN_USE_NOTIFICATION_ID);
    }

    private void showTimesUpNotification(final Context context) {
        for (Countdown timerObj : timersInTimesUp()) {
            showTimesUpNotification(context, timerObj);
        }
    }

    private void showTimesUpNotification(final Context context, Countdown timerObj) {
        // Content Intent. When clicked will show the timer full screen
        PendingIntent contentIntent = PendingIntent.getActivity(context, timerObj.getTimerId(),
                new Intent(context, MainActivity.class).putExtra(
                        TIMER_INTENT_EXTRA, timerObj.getTimerId()),
                PendingIntent.FLAG_UPDATE_CURRENT);

        // Add one minute action button
        PendingIntent addOneMinuteAction = PendingIntent.getBroadcast(context, timerObj.getTimerId(),
                new Intent(NOTIF_TIMES_UP_PLUS_ONE)
                        .putExtra(TIMER_INTENT_EXTRA, timerObj.getTimerId()),
                PendingIntent.FLAG_UPDATE_CURRENT);

        // Add stop/done action button
        PendingIntent stopIntent = PendingIntent.getBroadcast(context, timerObj.getTimerId(),
                new Intent(NOTIF_TIMES_UP_STOP)
                        .putExtra(TIMER_INTENT_EXTRA, timerObj.getTimerId()),
                PendingIntent.FLAG_UPDATE_CURRENT);

        // Notification creation
        Notification notification = new NotificationCompat.Builder(context)
                .setContentIntent(contentIntent)
//                .addAction(R.drawable.ic_menu_add,
//                        context.getResources().getString(R.string.timer_plus_1_min),
//                        addOneMinuteAction)
                .addAction(
                        android.R.drawable.ic_menu_close_clear_cancel,
                        context.getResources().getString(R.string.timer_done),
                        stopIntent)
                .setContentTitle(timerObj.getLabel())
                .setContentText(context.getResources().getString(R.string.timer_times_up))
                .setSmallIcon(R.drawable.stat_notify_timer)
                .setOngoing(true)
                .setAutoCancel(false)
                .setPriority(NotificationCompat.PRIORITY_MAX)
                .setDefaults(NotificationCompat.DEFAULT_LIGHTS)
                .setWhen(0)
                .setCategory(NotificationCompat.CATEGORY_ALARM)
                .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
                .setLocalOnly(true)
                .build();

        // Send the notification using the timer's id to identify the
        // correct notification
        NotificationManagerCompat.from(context).notify(
                timerObj.getTimerId(), notification);
        LogUtils.d(TAG, "Setting times-up notification for "
                + timerObj.getLabel() + " #" + timerObj.getTimerId());
    }

    private void cancelTimesUpNotification(final Context context) {
        for (Countdown timerObj : timersInTimesUp()) {
            cancelTimesUpNotification(context, timerObj);
        }
    }

    private RealmResults<Countdown> timersInTimesUp() {
        return mRealm.where(Countdown.class)
                .equalTo("state", Countdown.STATE_TIMESUP)
                .findAll();
    }

    private void cancelTimesUpNotification(final Context context, Countdown timerObj) {
        NotificationManagerCompat notificationManager = NotificationManagerCompat.from(context);
        notificationManager.cancel(timerObj.getTimerId());
        LogUtils.d(TAG, "Canceling times-up notification for "
                + timerObj.getLabel());
    }

    public long getTimesupTime(Countdown countdown) {
        return countdown.getStartTime() + countdown.getOriginalLength();
    }

    public static void updateTimerState(Context context, Countdown countdown, String action) {
        final Intent i = new Intent();
        i.setAction(action);
        i.putExtra(TIMER_INTENT_EXTRA, countdown.getTimerId());
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            // Make sure the receiver is getting the intent ASAP.
            i.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        }
        context.sendBroadcast(i);
    }
}
